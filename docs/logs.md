...
## 2025-12-12 11:47 北京时间
- 操作：账本前端新增“信用卡”模块，基于欠款锚点与交易流水自动追踪招商/广发信用卡欠款及潜在利息，移除旧的手工广发欠款估算。
- 新增点：
  - 页面加入「💳 信用卡」展示区，每张卡显示当前欠款、潜在利息（预计/累计）、账单日、到期日与剩余天数，并提示估算假设与潜在低估情况。
  - 支持交易中的“欠款锚点”作为基准，后续还款/消费/收入自动滚动计算欠款，收入视为退款抵减；无法归因的还款会在 UI 警告。
  - 配置面板新增招商/广发的账单日、到期日、日利率字段，默认 19/7、2/22、日利率 0.0005 并写入 localStorage。
- 删除点：移除配置中的 gf_debt 手填框及顶部“广发欠款（估算）”展示，不再依赖手工输入金额。
- 修改点：
  - 欠款计算改为“最新欠款锚点 + 锚点后交易增减”全正数体系，消费/年费等记为正向欠款，还款与收入（退款/返现）记为负向欠款，手续费仍按余额口径不进欠款。
  - 利息改为模式2：从账单日次日起（若锚点更晚则从锚点日）按“每日期末欠款×日利率”滚动累积，展示当前累计 ZZ 与若到期未全额还清时的预计 YY。
- 风险点：
  - 若缺少欠款锚点则无法计算欠款与利息；同日多笔交易依赖 tx_id 顺序，错误的序号可能导致滚动金额不准。
  - 锚点日期晚于账单日次日会低估利息，已在前端提示但需使用者知悉。
- 建议：
  - 新增或更新欠款锚点时记录正确的 tx_id 顺序，确保同日流水按录入先后排序。
  - 到期前若全额还清则利息应为 0，估算值仅供参考；如有手续费记入信用卡账户会被计入欠款，可按需调整分类或账户。
- 原计算/逻辑：广发欠款通过配置面板手填 gf_debt，并以固定月息约千五展示，无交易滚动与利息按日累积的估算。
- 改动后计算/逻辑：每张卡取最新“欠款锚点”金额为基准，按 tx.date 与 tx_id 顺序将锚点后的交易分为消费等（欠款增加）、还款（欠款减少，支持描述归因卡种）、收入退款（欠款减少）三类叠加；利息按账单日次日起（或锚点日）逐日累加 max(当日末欠款,0)×日利率，累计到今天为 ZZ，累计到下一到期日为 YY。
...
## 2025-12-12 13:26 北京时间
- 操作：修复信用卡欠款模块的 is_xiaoe 过滤与欠款显示，并补充利息提示细节。
- 新增点：欠款为负时金额展示钳制为 0，新增“已结清/溢出 ¥xx.xx”提示；锚点晚于起算日的利息提示包含具体日期。
- 删除点：无。
- 修改点：is_xiaoe 过滤改为显式判断 Number(tx.is_xiaoe)===1 才剔除；信用卡卡片使用 metric-value 样式并将分隔线改为固定 1px 实线；提示文案更新为“锚点晚于本期起算日（YYYY-MM-DD），累计利息可能低估”。
- 风险点：新增 border-color 变量与边框写法，需确认深浅色主题下边框颜色保持一致；欠款钳制为 0 可能与实际负债不符，需依赖提示识别溢出。
- 建议：上线后验证 is_xiaoe 返回 "0"/false 时交易仍参与欠款滚动；在超额还款场景查看 UI 是否显示 0 元并附带溢出金额提示，确认边框样式正常。
- 原计算/逻辑：欠款过滤使用 !tx.is_xiaoe，字符串 "0" 也会被视为 true 进而被剔除；欠款展示直接输出负值；锚点晚于起算日的提示缺少具体日期。
- 改动后计算/逻辑：仅当 Number(tx.is_xiaoe)===1 时才过滤小鹅订单；欠款金额展示钳制至 0 并以提示标出溢出金额；锚点晚于起算日时在提示中附带具体日期以便核对。

## 2025-12-12 13:32 北京时间
- 操作：修正信用卡利息累积循环的起算游标，避免遗漏账单起算日至锚点日之间的潜在利息计算。
- 新增点：无。
- 删除点：无。
- 修改点：将利息计算循环的 cursor 起点从锚点日期调整为锚点日期与起算日之间的较早日期（Math.min(anchorDate, interestStartDate)），确保未来调整起算策略时不会漏算起算日至锚点日区间的利息。
- 风险点：若后续引入不同利息起算逻辑需同步确认 cursor 对齐规则，但当前更改为取最早日期已兼容常见场景。
- 建议：验证包含锚点晚于账单起算日的账期案例，确认累计利息与预计利息均覆盖起算日至锚点日间的欠款基数。
- 原计算/逻辑：利息累积循环从锚点日期起步，若未来起算日早于锚点且未同步调整循环起点，可能漏算起算日至锚点日间利息。
- 改动后计算/逻辑：循环起点取锚点与起算日的最小值，即使锚点在账单起算日之后亦能覆盖整个计息区间，避免遗漏。
