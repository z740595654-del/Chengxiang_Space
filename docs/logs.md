...
## 2026-01-21 17:52 北京时间
- 操作：新增旅行账本页面与入口，并在主账分析中排除 book=旅行，信用卡欠款改为统计全部账本。
- 新增点：travel.html 提供行程管理、日均/类别/Top10 图表及交易选择器；index.html 新增旅行入口卡片；账本录入 book 支持主账/旅行下拉。
- 删除点：无。
- 修改点：账本统计口径改为仅统计 book=主账；信用卡欠款/锚点统计去掉 book 过滤，纳入全部账本交易；新增旅行板块本地缓存结构（travel_itineraries_v1）。
- 风险点：旅行交易不再进入主账统计，若用户依赖旧口径需要适应；信用卡欠款合并所有账本可能导致数值变动。
- 建议：更新后进行一次手动自测，确认旅行交易仅影响总余额与信用卡欠款，不影响主账预算；并验证行程切换与桶标签持久化是否正常。
- 原计算/逻辑：主账统计使用所有非 meta 交易；信用卡欠款仅统计 book=主账 的交易。
- 改动后计算/逻辑：主账统计仅使用 book=主账 且非 meta 的交易；信用卡欠款与欠款锚点统计移除 book 过滤，纳入全部账本交易。
