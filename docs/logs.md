...
## 2025-12-11 16:18 北京时间
- 操作：调整 burn chart 可见天数的计算逻辑，优先按本期最后一笔消费日期截断 X 轴。
- 新增点：无。
- 删除点：无。
- 修改点：
  - visibleDays 计算改为先遍历 daily 找最后有支出的索引，若有消费则截断到该日；无消费时才按今天与账期起止回退计算，并确保至少为 1 天。
  - totalDays 判定提前于 daily 可视化逻辑，防止无效账期导致渲染异常。
- 风险点：若存在日期超出账期的异常数据不会计入 daily，X 轴仍按账期或今日回退，可能与期望不符。
- 建议：导入数据时保证日期在账期范围内，若需展示超期消费可调整账期配置。
- 原计算/逻辑：visibleDays 始终按今天与账期结束比较，账期已结束或今天晚于结束日时与 totalDays 相同，导致无论是否有消费都渲染完整期。
- 改动后计算/逻辑：先按 daily 找到最后一笔消费日期截断 X 轴；仅在完全无消费时使用今天与账期范围计算 visibleDays，并下限为 1 天，理想线分母依旧使用 totalDays。
...
## 2025-12-11 16:36 北京时间
- 操作：将 burn 图表的 X 轴改为 Plotly 的离散类别轴，避免日期被当作时间轴压缩或补间。
- 新增点：无。
- 删除点：无。
- 修改点：burnChart 布局的 xaxis 添加 type="category" 并保留原有刻度角度与网格设定。
- 风险点：类别轴不再自动推断缺失日期，若后续需要时间线插值需另行处理。
