...
## 2025-12-09 09:33 北京时间
- 操作：按 Wrangler 要求将 wrangler.toml 的 routes 调整为字符串数组，避免对象缺少 zone 信息导致部署报“Expected routes to be an array of strings or objects…”。
- 新增点：无。
- 删除点：无。
- 修改点：routes 由对象数组 `{ pattern = "czbpght.cn/food*" }` 改为字符串数组 `"czbpght.cn/food*"`，不改动 Worker 逻辑或其他配置。
- 风险点：若未来需要在同一配置中添加其他路由/域名，需确保继续采用 Wrangler 支持的字符串或完整对象格式。
- 建议：执行 `npx wrangler deploy` 或 `npx wrangler publish` 确认不再出现 routes 结构校验错误，线上访问 /food 与 /food.html 行为保持不变。
