...
## 2025-12-10 17:08 北京时间
- 操作：提交单笔记账时去除前端随机生成 tx_id，改为交由后端按当日序号生成。
- 新增点：无。
- 删除点：移除前端生成随机 4 位尾号的 tx_id 逻辑。
- 修改点：提交 payload 不再包含 tx_id，由后端按当日已存在数量递增生成 YYYYMMDD-序号。
- 风险点：前端完全依赖后端生成编号，若接口异常可能导致提交失败或编号未返回；并发场景需依赖后端序号分配避免冲突。
- 建议：发布后手动录入同日多笔交易，确认返回的 tx_id 递增且列表排序正常；必要时在前端提示返回的 tx_id 以便核对。
- 原计算/逻辑：前端使用日期 + 随机四位数拼接 tx_id，可能与后端排序或并发编号冲突。
- 改动后计算/逻辑：前端提交不带 tx_id，后端根据当日交易数自动生成补零递增的序号，确保与后端排序一致。
