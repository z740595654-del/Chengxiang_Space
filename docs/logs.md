...
## 2025-12-08 17:20 北京时间
- 操作：为干饭转盘接口增加数据库绑定检查并集中 food_state 表名常量，避免因绑定缺失或表名不一致导致同步失败。
- 新增点：Worker 在缺少 MY_DB 绑定时返回明确的配置缺失错误，便于排查部署问题。
- 删除点：无。
- 修改点：food_state 表名提取为常量并通过 ensureFoodTable 统一创建；处理逻辑在执行查询前先校验数据库绑定。
- 风险点：缺少 MY_DB 绑定时会直接返回 500，需确保部署环境已正确配置 D1 数据库。
- 建议：发布前核对 wrangler.toml 与线上环境的 D1 绑定一致，若同步失败可根据返回错误快速定位。
- 原逻辑：直接对 env.MY_DB 调用 prepare，若绑定缺失会抛出泛化异常且无法确认根因。
- 改动后逻辑：先检测 MY_DB 是否存在，不存在时给出明确错误提示，并统一确保 food_state 表存在后再读写。
