...
## 2025-12-12 15:08 北京时间
- 操作：在信用卡“现实口径 A”模块新增按天历史利息展示，补充今日利息与至今累计利息，并保持到期日整期潜在利息提示。
- 新增点：
  - 新增 calcCardInterestDaily，基于锚点金额与流水逐日滚动，输出每日至欠款、当日利息与累计利息。
  - renderCreditCards 新增“今日潜在利息”“历史累计利息（至今日）”文案，并提供按天历史明细折叠表，仅覆盖 interestStart..today 区间。
- 删除点：无。
- 修改点：
  - 潜在利息（至到期日）改为复用每日累加结果的总额；历史利息与今日利息同样使用每日滚动结果，截断至今日或到期日。
  - interestStart 仍取账单周期起点与欠款锚点中较晚者，历史区间上限取 today 与 dueDate 的较小值，确保过期时不越界。
- 风险点：每日遍历区间在流水密集或区间跨度较大时有一定性能消耗；若交易数据缺失锚点或排序异常，明细可能为空或金额偏差。
- 建议：验证近半年内账期在界面渲染性能与金额正确性，关注锚点晚于起算日时的低估提示是否合理；如需优化可考虑在超长区间做分段或限流。
- 原计算/逻辑：潜在利息仅通过 calcCardInterest 在 interestStart..dueDate 上累加，不提供今日/历史拆分，也不展示逐日明细。
- 改动后计算/逻辑：使用 calcCardInterestDaily 先归集 startDate 之前的流水再逐日滚动，同步产出每日余额/利息/累计；今日利息取历史数组最后一日的当日利息，历史累计取至今日总额，整期潜在利息取到期日累计值，明细折叠仅展示 interestStart..today 区间。

## 2025-12-12 14:43 北京时间
- 操作：信用卡潜在利息计算起算日与锚点对齐，修正锚点晚于账期起算日时的预测口径。
- 新增点：当欠款锚点晚于账单周期起算日时，潜在利息的起算日自动顺延至锚点日期，避免对未可恢复的历史欠款做推算。
- 删除点：无。
- 修改点：
  - renderCreditCards 中潜在利息计算起点从固定使用周期起算日，调整为周期起算日与锚点日期的较晚者；提示文案保持“锚点晚于本期起算日…预计利息可能低估”。
  - 计算逻辑不再对锚点前余额计息，预测更贴近可追溯区间。
- 风险点：锚点过晚时潜在利息将低估，需提醒用户补齐锚点或确认欠款区间。
- 建议：更新锚点到账期起始或补充中间流水后再查看预测，确保覆盖完整欠款。 
- 原计算/逻辑：潜在利息始终从账单周期起算日开始累积，即便欠款锚点晚于起算日也会将锚点前余额计入预测。
- 改动后计算/逻辑：潜在利息从账单周期起算日与欠款锚点日期中的较晚日期开始滚动，仅对可追溯区间计息，并保留锚点晚于起算日的低估提示。

## 2025-12-12 13:47 北京时间
- 操作：为 ledger.html 新增信用卡“现实口径 A”模块，固定招商/广发卡账单日、到期日与日利率，并基于欠款锚点与流水计算当期欠款与预计利息。
- 新增点：
  - 写死招商信用卡（账单日 19、到期日 7、日利率 0.0005）与广发信用卡（账单日 2、到期日 22、日利率 0.0005）参数，界面展示不可修改。
  - 新增信用卡卡片，显示欠款、下一到期日、若未全额还清的预计利息，并在锚点晚于本期起算日时提示可能低估。
- 删除点：取消信用卡参数在配置面板的可编辑入口（保持写死，不再读取/覆盖本地配置）。
- 修改点：
  - 到期日前累计利息固定展示为 0，仅提供“若到期未全额还清”的滚动法（账单日次日起按日欠款 × 日利率累计至到期日）预计值。
  - 欠款计算以最新欠款锚点为基准，后续消费累加、还款抵减，按照交易日期与 tx_id 顺序滚动。
- 风险点：
  - 欠款锚点需以“锚点”分类标记且账户名称匹配卡名，否则计算将从 0 起算；同日交易顺序依赖 tx_id，异常编号可能影响欠款滚动。
  - 账单/到期日写死，若银行政策变动需手动更新代码；滚动利息未考虑超过到期日后的违约复利情形。
- 建议：
  - 用实际流水验证锚点 + 还款的欠款滚动是否符合预期，重点核对用户提供的 2025-12-02/12-11 场景。
  - 后续如需支持不同卡片或利率调整，可在代码中扩充 CREDIT_CARDS 常量。
- 原计算/逻辑：前端无信用卡利息滚动与账期内欠款自动计算，依赖手填估算且到期前不会区分预计利息与实际为零的口径。
- 改动后计算/逻辑：
  - 固定卡片参数，按账单日确定本期起算日、到期日，下一到期日前累计利息展示为 0；同时计算若到期未全额还清的潜在利息（账单日次日起按当日欠款 × 日利率累计至到期日）。
  - 欠款从最新锚点金额起，依交易日期/tx_id 顺序将非锚点流水记为欠款增加，分类为“还款”的流水记为欠款减少，计算至今天展示当前欠款。
...
## 2025-12-12 14:07 北京时间
- 操作：信用卡模块切换为“现实口径 A（不制造焦虑）”，固定写死招商/广发参数并移除配置面板与本地存储覆盖，利息展示改为到期日前固定 0 与单独的潜在利息行。
- 新增点：
  - 信用卡卡片新增“当前应计利息：¥0.00”与“潜在利息”两行展示，账单日未到时改为“本期未出账，利息预测不展示”。
  - 欠款流水计算仅接受主账且未标记小鹅的交易，锚点识别限定一/二级分类为“欠款锚点”，退款/返现与还款按绝对值抵扣欠款。
- 删除点：配置面板移除信用卡相关输入与广发欠款估算展示，资产卡不再显示广发欠款估值。
- 修改点：锚点晚于起算日的提示仅影响潜在利息；欠款滚动逻辑改为按绝对值累加消费/年费，扣减还款与收入类退款；账单日之前不计算潜在利息。
- 风险点：老的本地存储字段（如 gf_debt）将被忽略，需确认不会影响其他页面；交易分类若不符合“收入/还款/欠款锚点”口径可能导致欠款方向偏差。
- 建议：按回归用例验证招商/广发欠款与利息展示是否符合“到期前利息为 0、账单未出不预测”；如有特殊分类命名请在数据侧对齐“收入/还款/欠款锚点”。
- 原计算/逻辑：信用卡锚点通过 isMetaTx 泛化判断，锚点后流水按原始金额正负累加，未区分退款抵扣；到期前仍展示预计利息且可能在账单未出前计算招商利息。
- 改动后计算/逻辑：锚点只认分类为“欠款锚点”，仅主账且非小鹅交易计入；锚点后消费/年费等按绝对值增加欠款，还款与账户为该卡的收入/退款按绝对值扣减；到期日前应计利息固定 0，账单未出时潜在利息不展示，账单已出后才按模式 2 滚动预测潜在利息。

## 2025-12-12 14:35 北京时间
- 操作：修正信用卡“现实口径 A”周期与利息展示，确保周期覆盖上一个账单日后所有流水并始终展示潜在利息预测。
- 新增点：潜在利息文案补充“仅基于已入账流水；未来新增消费/还款会改变预测”的提醒。
- 删除点：移除“本期未出账，利息预测不展示”的分支，避免账单日前不计算潜在利息。
- 修改点：
  - 账单周期起算日改为上一个账单日+1（通过遍历上/本/下月账单日并选择下一到期日），避免周期落在未来导致漏算上期消费。
  - 潜在利息始终按模式 2 计算并展示，锚点晚于起算日提示保持仅影响预测值。
- 风险点：周期推算改动依赖账单日与到期日组合，需留意跨月到期卡片是否覆盖所有边界；潜在利息恒算可能在无流水时展示 0 预测，需要确认文案接受度。
- 建议：在 2025-12-12 等跨期场景验证招商（19/7）周期为 2025-11-20~2026-01-07、广发（2/22）周期为 2025-12-03~2025-12-22，确认欠款与预测均覆盖上期消费；上线后复核利息文案对用户的预期影响。
- 原计算/逻辑：账单周期从“本月账单日+1”起算，可能落到未来导致漏掉上期消费；账单日前不计算潜在利息，直接显示“本期未出账，利息预测不展示”。
- 改动后计算/逻辑：按上/本/下月账单日推算“下一到期日”，取其前一账单日+1为周期起点，确保覆盖上期消费；无论是否出账均按模式 2 计算潜在利息并展示 0.00/提示低估风险。

## 2025-12-12 14:51 北京时间
- 操作：修正规整日期函数 buildDateForMonth 的月溢出与月末裁剪方式，确保跨月跨年账单周期推算稳定。
- 新增点：无。
- 删除点：无。
- 修改点：先让 JS 规范化 year/month 再按当月天数裁剪 day，避免直接 new Date 溢出到下一月导致账单周期计算偏移。
- 风险点：无明显风险，需在包含 31 天与跨年账期场景验证周期覆盖是否正确。
- 建议：回归信用卡周期推算（尤其跨年时段）确保账单日、到期日与周期起算日匹配预期。
- 原计算/逻辑：new Date(year, month, day) 若 day 超出当月天数会滚动到下月，导致账单日溢出时周期可能偏移到下个月末。
- 改动后计算/逻辑：先以 month 溢出后的实际年月为基准获取当月最后一天，并将 day 限制在 1~lastDay 之间，再构造日期，避免溢出到下一月。

## 2026-01-04 11:38 北京时间
- 操作：调整 ledger.html 顶部资产卡余额为现金口径并修正往来方向判定。
- 新增点：增加信用卡账户集合（含“信用卡”别名）与往来方向关键词，支持描述以 “+/-” 前缀强制转入/转出；余额提示与控制台输出新增收入/支出/往来净额及忽略信用卡消费的调试信息。
- 删除点：无。
- 修改点：现金余额仅统计锚点后非元数据且非小鹅流水；收入仅非信用卡账户计入现金，信用卡消费在其他类别中不再扣减现金；还款统一视为现金流出；往来按描述关键词或前缀判定方向；无锚点提示改为“未设置基准余额记录”。
- 风险点：往来方向依赖描述关键词或人工前缀，若描述缺乏特征可能仍按转出处理；新增信用卡账户若未加入集合会被当作现金账户计入。
- 建议：导入后在控制台查看余额调试对象，确认收入/支出/往来净额与预期一致；如添加新的信用卡账户名称需同步加入集合。
- 原计算/逻辑：锚点后收入全部计入现金流入、其余一律扣减，导致信用卡消费与往来转入被当作现金流出，且未剔除小鹅记录。
- 改动后计算/逻辑：锚点后仅统计非元数据且非小鹅流水；收入仅非信用卡账户计入现金；还款一律扣减；往来依据描述关键词或 “+/-” 前缀判方向并计入净额；信用卡消费在其他类别中不影响现金余额，并单独累加忽略值用于提示。
## 2026-01-22 09:21 北京时间
- 操作：在账本页面新增旅行行程卡片与旅行分析，并将主账统计限定为 book="主账"，信用卡欠款统计覆盖全部 book。
- 新增点：账本录入区新增“主账/旅行”账本下拉选择；旅行卡片支持行程管理、交易选择弹窗、按日折线/类别柱状/Top10 支出分析，并使用 localStorage 缓存行程与桶标签。
- 删除点：无。
- 修改点：主账预算/分类/图表/热力图等统计改为仅使用 book="主账" 的非 meta 交易；信用卡欠款锚点与事件统计移除 book 限制以覆盖全部账本。
- 风险点：若历史交易未填写 book 字段，可能需手动补为“主账”以继续参与主账统计；旅行分析依赖 tx_id 匹配，缺失 tx_id 的旅行交易无法被行程选择。
- 建议：补录历史交易的 book 字段并在刷新后确认主账统计一致；为旅行账本交易保持稳定 tx_id 以便行程复用。
- 原计算/逻辑：主账统计直接使用全部非 meta 交易，不区分 book；信用卡欠款锚点与流水仅统计 book="主账"。
- 改动后计算/逻辑：主账统计过滤为 book="主账" 的非 meta 交易，旅行账本交易被剔除并在旅行卡片单独分析；信用卡欠款统计覆盖全部 book（含主账与旅行）。

## 2026-01-22 10:08 北京时间
- 操作：调整旅行卡片分析区，仅保留按类别柱状图与 Top10，并新增平均每日消费的小字展示。
- 新增点：旅行卡片汇总区新增“平均每日消费”小号展示，按选中支出交易的自然日跨度计算。
- 删除点：移除旅行卡片“按日”图表与相关日期序列/每日聚合渲染逻辑。
- 修改点：旅行分析不再渲染按日折线图，日均计算改为最小/最大支出日期的自然日跨度（含首尾）后计算均值。
- 风险点：若支出交易日期存在格式异常导致日期解析失败，平均每日消费会退回默认天数 1，需关注数据规范性。
- 建议：确保旅行账本交易日期为 YYYY-MM-DD 格式，以保证平均每日消费口径准确。
- 原计算/逻辑：按支出交易生成连续日期序列并绘制按日折线图，日均按序列天数计算。
- 改动后计算/逻辑：取消按日折线图与日期序列；平均每日消费改为以支出交易最小/最大日期的自然日跨度（含首尾）计算，若无支出交易则天数为 1。

## 2026-01-22 11:21 北京时间
- 操作：修正大额消费统计覆盖全账本，并让录入区账本下拉动态包含主账/旅游/3D打印与历史账本。
- 新增点：录入区账本下拉在加载交易后按去重账本动态重建，强制包含“主账/旅游/3D打印”。
- 删除点：无。
- 修改点：大额消费卡片的数据源改为全账本非 meta 交易的 B 口径，而非仅主账；大额标记刷新时同样使用全账本数据。
- 风险点：若交易数据中存在异常 book 值，会被下拉直接展示，需留意数据规范性。
- 建议：标记一笔非主账（如 3D打印）交易为大额，确认大额卡片能展示；新增交易时确认下拉仍默认“主账”。
- 原计算/逻辑：大额消费候选与已标记列表基于 getStatTransactions()（仅主账）筛选 B 口径。
- 改动后计算/逻辑：大额消费候选与已标记列表改为基于 filterBBase(appState.allNonMetaTxs)（全账本非 meta）筛选 B 口径。
